...
interface ReadathonFieldProps {
  value?: string | null
  onChange: (value: string | null) => void
  disabled?: boolean
}

export default function ReadathonField({ value, onChange, disabled = false }: ReadathonFieldProps) {
  const [isReadathon, setIsReadathon] = useState(!!value)
  ...
  useEffect(() => {
    if (query.length > 0) {
      fetch(`/api/user/readathons?q=${encodeURIComponent(query)}`)
        .then(res => res.json())
        .then(data => {
          if (data.readathons) {
            setSuggestions(data.readathons.map((readathon: { name: string }) => readathon.name))
          }
        })
        .catch(console.error)
    } else {
      // Fetch all when no query
      fetch('/api/user/readathons')
        .then(res => res.json())
        .then(data => {
          if (data.readathons) {
            setSuggestions(data.readathons.map((readathon: { name: string }) => readathon.name))
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <Switch
          ...
          disabled={disabled}
          className={`${
            isReadathon ? 'bg-indigo-600' : 'bg-gray-200 dark:bg-gray-700'
          } relative inline-flex h-6 w-11 items-center rounded-full transition-colors`}
        >
          <span className="sr-only">Read-a-thon toggle</span>
          <span
            className={`${
              isReadathon ? 'translate-x-6' : 'translate-x-1'
          ...
          <Combobox value={value || ''} onChange={onChange} disabled={disabled}>
            <div className="relative">
              <Combobox.Input
                ...
                placeholder="Enter read-a-thon name..."
                maxLength={100}
              />
